...
| `SectionRunThroughPanel` | Surfaces section run-throughs that are currently due, or locked/grayed the day before they will be, plus combined (section-pair) run-throughs once unlocked. **Since Pass 49, single-section run-throughs are a repeating gate** (due → not due → due again as the section's slowest chunk picks up more sessions), not a permanent once-unlocked panel — see [Algorithms.md](Algorithms.md#section-run-throughs). |
