...
| `TechniqueTab` (Pass 101) | The Technique page, app-level (not scoped to the open piece), second in the sidebar after Master Agenda with a hairline divider (`.nav-divider`) before the piece-scoped items. Eyebrow "Technique", h1 "Scales and arpeggios", an "Add scale or arpeggio" button (`AddTechniqueItemForm`), and three subtabs: **Today** (`TechniquePanel` with the circle-of-fifths walk hint), **Library** (`TechniqueLibrary`: filter, three sorts, row editor, the check-octaves keep/start-fresh prompt inside the row) and **Methods** (`TechniqueMethods`). `TechniquePanel` is deliberately standalone — Pass 102 reuses it on Master Agenda and Daily Practice with `variant="shared"`. Every change goes through `App.jsx`'s `handleTechnique*` handlers and `updateTechnique`, never `setPieces`/`updatePiece`. Layout and copy follow [the mockup](mockups/technique-practice.html); behavior follows [Technique-Practice.md](Technique-Practice.md). |
