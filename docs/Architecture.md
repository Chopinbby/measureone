...
| `SettingsTab` | View mode: read-only summary + Edit/Delete/Add-new-piece/Export/Import, plus a "Practice status" panel (Pause/Archive — Archive gated on `isPlanActuallyComplete`), **since a later session, "Mark as learned elsewhere"** — sets `piece.markedLearnedElsewhere`, an unconditional override for a piece finished away from the app, freely reversible ("Undo") — and, **since Pass 91 (experimental v1), an always-visible "Focus spots" panel**: an On/Off toggle for `piece.troubleSpotsEnabled` and, only while On, a `NumberInput` for `piece.troubleSpotDefaultMinutes` (the minimum-practice-time gate a `FocusSpotCard` enforces before either of its actions enables — see [Algorithms.md](Algorithms.md#focus-spots-v1)). Edit mode reuses the shared field-editor components; "Save changes" is disabled on a blank piece name or zero total measures (mirroring the Wizard's own `canAdvance()` gate, added in the same later session) in addition to the pre-existing blank-work-title check. **Shown only while `isInRevival(piece)`:** a "Revival settings" panel holding the tempo ladder's starting-BPM field (`tempoLadderStartFraction`, derived from `BPM / targetBPM` on commit) — collected once at revival entry (`RevivalEntryModal`) and editable from here afterward; this is the one place that value can be changed post-entry (briefly removed from the app entirely during Pass 88, then put back here on same-session follow-up — see [Decisions.md](Decisions.md#revival)). |
